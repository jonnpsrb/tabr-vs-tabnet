        ...
        *,
        ...
        def deq_func(x):
            for block in self.blocks1:
              x = x + block(x)
            return x
        x_pred, _ = self.deq(deq_func, x, f_solver='broyden', f_max_iter=20)
        #x = x[-1]
        x = [self.head(x) for x in x_pred]
        x = x[-1]
        return x
...
def main(
    ...
    while epoch < ConfigUser.n_epochs:
        ...
        model.train()
        epoch_losses = []
        for batch_idx in lib.make_random_batches(train_size, ConfigUser.batch_size, device):
