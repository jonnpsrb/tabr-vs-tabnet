...
class DEQModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, solver, **kwargs):
        super(DEQModel, self).__init__()
        self.deq_block = DEQBlock(input_dim, hidden_dim)
        self.solver = solver
        self.kwargs = kwargs

    def forward(self, x):
        with torch.no_grad():
            z, self.forward_res = self.solver(lambda z: self.deq_block(z), x.clone(), **self.kwargs)
        z = self.deq_block(z)
        
        z = z.requires_grad_(True)  # Ensure z requires gradient
        z0 = z.clone().detach().requires_grad_()
        f0 = self.deq_block(z0)
...
def broyden_solver(f, x0, max_iter=20, tol=1e-5, alpha=1.0):  # Reduced max_iter
    bsz, d = x0.shape
    x = x0.clone().requires_grad_().to(x0.device)
    fx = f(x)
    r = fx - x
    I = torch.eye(d, device=x.device).unsqueeze(0).expand(bsz, d, d)
    B = I.clone()

    for _ in range(max_iter):
        dx = torch.linalg.solve(B, -r.unsqueeze(-1)).squeeze(-1)
        x = x + alpha * dx
        fx = f(x)
        r = fx - x
        if torch.norm(r) < tol:
            break
        u = fx - x - dx
        v = torch.bmm(B, dx.unsqueeze(-1)).squeeze(-1)
        B = B + (u - v).unsqueeze(-1) * dx.unsqueeze(1) / torch.sum(dx * dx, dim=1, keepdim=True).unsqueeze(-1)
...
class Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Integrasi DEQ dengan solver Broyden
        self.deq_model = DEQModel(d_main, d_block, broyden_solver, max_iter=50, tol=1e-5, alpha=1.0)
