...
class Config:
    ...
    def __post_init__(self):
        ...
        if 'callbacks' in self.fit:
            early_stopping_present = any(
                callback.get('name') == 'early_stopping' 
                for callback in self.fit['callbacks']
            )
            assert early_stopping_present, 'early_stopping callback must be included in callbacks'
        else:
            raise ValueError('No callbacks defined in fit configuration')
